...
<body>
    ...
    if($result = $mysqli->query('SELECT  d.name_dealer,  c.name, c.lastname,  cars.mark, cars.model, cars.color FROM dealers AS d LEFT JOIN clients AS c ON d.id_client = c.id_client INNER JOIN cars ON d.id_car = cars.id_car ORDER BY d.name_dealer;')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит

        printf("<h3>Звіт - Список дилерів, клієнтів і машин: </h3><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>Назва дилера</th><th>Ім'я</th><th>Призвище</th><th>Марка</th><th>Модель</th><th>Колір</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>",  $row['name_dealer'],  $row['name'], $row['lastname'], $row['mark'], $row['model'], $row['color'] ); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
