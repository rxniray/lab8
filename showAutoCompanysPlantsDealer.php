...
<body>
    ...
    if($result = $mysqli->query('SELECT  cc.name_car_company,  p.name_plant,  d.name_dealer FROM car_companys AS cc LEFT JOIN plants AS p ON cc.id_plant = p.id_plant INNER JOIN dealers AS d ON cc.id_dealer = d.id_dealer ORDER BY cc.name_car_company;')) {  

        printf("<h3>Звіт - Список компаній, заводів і дилерів: </h3><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>Назва компанії</th><th>Назва заводу</th><th>Назва дилера</th></th>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['name_car_company'],  $row['name_plant'],  $row['name_dealer']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
